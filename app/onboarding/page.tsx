...
function CheckChip({
  ...
  return (
    ...
    >
      ...
      >
        {checked && <span className="text-[#141414] text-[10px] font-bold">✓</span>}
      </span>
      {label}
    </button>
  );
...
function Step1({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  return (
    <div className="space-y-5">
      ...
      <div>
        <Label>Age</Label>
        <TextInput placeholder="e.g. 35" value={data.age} onChange={(v) => set('age', v)} type="number" />
      </div>
      <div>
        <Label>Gender</Label>
        <div className="flex gap-3">
...
const INCOME_RANGES = ['< ₹10k', '₹10k–25k', '₹25k–50k', '₹50k–1L', '₹1L–2L', '> ₹2L'];
const SAVINGS_RANGES = ['< ₹10k', '₹10k–50k', '₹50k–1L', '₹1L–5L', '₹5L–10L', '> ₹10L'];
...
function Step4({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  const EXPENSE_RANGES = ['None', '< ₹2k', '₹2k–5k', '₹5k–10k', '₹10k–20k', '> ₹20k'];
  ...
  const expenses: { key: keyof FormData; label: string; sub: string }[] = [
    ...
    { key: 'utilities', label: 'Utilities', sub: 'Electricity, water & maintenance' },
      ...
      {expenses.map((e) => (
        <div key={e.key}>
          <div className="mb-2">
            ...
            <span className="font-mono text-[11px] text-[#555] ml-2">{e.sub}</span>
          </div>
          <div className="flex flex-wrap gap-2">
...
function Step5({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  return (
    <div className="space-y-6">
      <YesNo label="Do you currently save money?" value={data.savesMoney} onChange={(v) => set('savesMoney', v)} />
...
function Step6({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  ...
  const toggle = (inv: string) => {
    ...
    set('investments', cur.includes(inv) ? cur.filter((i) => i !== inv) : [...cur, inv]);
  };

  return (
    <div className="space-y-6">
      <YesNo label="Have you invested before?" value={data.investedBefore} onChange={(v) => set('investedBefore', v)} />
...
function Step7({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  return (
    <div className="space-y-6">
      <YesNo label="Do you have health insurance?" value={data.healthInsurance} onChange={(v) => set('healthInsurance', v)} />
      <YesNo label="Do you have life insurance?" value={data.lifeInsurance} onChange={(v) => set('lifeInsurance', v)} />
    </div>
  );
}

function Step8({ data, set }: { data: FormData; set: (k: keyof FormData, v: any) => void }) {
  return (
    <div className="space-y-6">
      <YesNo label="Do you file income tax?" value={data.filesTax} onChange={(v) => set('filesTax', v)} />
      <YesNo label="Would you like to save on taxes?" value={data.wantsTaxSave} onChange={(v) => set('wantsTaxSave', v)} />
    </div>
  );
}
...
const STEP_META = [
  ...
  { title: 'Expense Snapshot', sub: 'Roughly how much do you spend each month?' },
  ...
  return (
    <div className="min-h-screen bg-[#141414] flex items-center justify-center px-4">
      ...
      >
        ...
        <h2 className="font-serif text-3xl text-white mb-4 leading-snug">{msg.headline}</h2>
        <p className="font-mono text-[14px] text-[#888] leading-relaxed mb-10">{msg.body}</p>
        <button
          onClick={onDone}
...
export default function Onboarding() {
  ...
  const canProceed = (): boolean => {
    if (step === 1) return !!(data.name && data.age && data.gender && data.caste);
    ...
    if (step === 7) return data.healthInsurance !== null && data.lifeInsurance !== null;
    ...
    <div className="min-h-screen bg-[#141414] flex flex-col items-center justify-start px-4 py-10 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-md px-8 py-8 shadow-2xl relative">
          ...
          <div className="mb-6">
            <h2 className="font-serif text-2xl text-white mb-1">{meta.title}</h2>
